...
import com.mbv.ticketsystem.airline.vietjet.VietjetPay.VietjetPayException;
...
public class VietjetStub {
	...
	private String chkPax1 = "";
	private String hdnInputPax1 = "";
	...
	private HashMap<String, String> createSearchParams(UpdateFarePriceCommand request,final String viewstate,final String viewstategen) throws Exception {
		try {
			...
			HashMap<String, String> params = new HashMap<String, String>() {
				{
					...
					put("lstCompanyList", "722ƒCTY VIET NHANH");
	...
	private void loginBookVietJet() throws Exception{
		...
		sessId = list.get(1);
		viewstategen = list.get(2);
		Connection.Response resPostLogin =vjBook.postProcess(urlLogin,vjBook.createParamsLogin(viewstate,viewstategen,username,password),sessId);
	...
	private void menuBookVietJet() throws Exception{
		...
		viewstate = list.get(0);	
		viewstategen = list.get(2);
		Connection.Response resPostMenu = vjBook.postProcess(urlMenu,vjBook.createPostMenuParams(viewstate,viewstategen),sessId);
	...
	private void searchBookVietJet(AirItinerary itinerary) throws Exception{
		...
		Connection.Response resPostSearch = vjBook.postProcess(urlSearch,vjBook.createSearchParams(request,viewstate,viewstategen),sessId);	
	...
	private String searchViewFlight(UpdateFarePriceCommand request) throws Exception{
		...
		viewstate = list.get(0);		
		viewstategen = list.get(2);
		Connection.Response resPostSearch = vjBook.postProcess(urlSearch,createSearchParams(request,viewstate,viewstategen),sessId);	
		urlResult = resPostSearch.url();	
		if (!urlResult.toString().contains("TravelOptions.aspx"))	{
			logger.error("VietjetBook: Post_Search Exception");
			throw new Exception("Post_Search Exception");
	...
	private void travelBookVietJet(AirItinerary itinerary) throws Exception{
		...
		Connection.Response resPostTravel = vjBook.postProcess(urlTravel,vjBook.createPostTravelParams(reference1,reference2,viewstate,viewstategen),sessId);
	...
	private boolean detailBookVietJet(AirItinerary itinerary,long sumPrices) throws Exception{
		...
		Connection.Response resPostDetail = vjBook.postProcess(urlDetail,vjBook.createPostDetailParams(passengerInfos,contactInfo,viewstate,viewstategen),sessId);
	...
	private void addonsBookVietJet(AirItinerary itinerary) throws Exception{
		...
		if(purchaseValue.equals("8"))
			resPostAddOns =  vjBook.postProcess(urlAddOns,vjBook.createPostAddOnsParams(roundTrip,adt_chd,extraServices,listAttrPasKg,listAttrPasKgBack,glagMeal,viewstate,viewstategen),sessId);
		else
			resPostAddOns =  vjBook.postProcess(urlAddOns,vjBook.createPostAddOnsParams2(roundTrip,adt_chd,extraServices,listAttrPasKg,listAttrPasKgBack,glagMeal,viewstate,viewstategen),sessId);
		// 
	...
	private void paymentsBookVietJet(AirItinerary itinerary) throws Exception{
		...
		viewstate = list.get(0);
		viewstategen = list.get(2);
		Connection.Response resPostPayments = vjBook.postProcess(urlPayments,vjBook.createPostPaymentsParams(viewstate,viewstategen),sessId);		
	...
	private void confirmBookVietJet(AirItinerary itinerary) throws Exception{
		...
		viewstate = list.get(0);	
		viewstategen = list.get(2);
		Connection.Response resPostConfirm = vjBook.postProcess(urlConfirm,vjBook.createPostConfirmParams(viewstate,itinerary,viewstategen),sessId);	
	...
	public boolean verifyBook(AirItinerary itinerary) throws Exception{
		logger.info("Vietjet check book: login");
		loginBookVietJet();	
		logger.info("Vietjet check book: menu");
		menuBookVietJet(); 
		logger.info("Vietjet check book: search");
		searchBookVietJet(itinerary);	
		logger.info("Vietjet check book: travel");
		travelBookVietJet(itinerary);
		logger.info("Vietjet check book: detail");
		detailBook(itinerary);			
		logger.info("Vietjet check book: addons");
		addonsBookVietJet(itinerary);	
		logger.info("Vietjet check book: payment");
		paymentsBookVietJet(itinerary);	
		logger.info("Vietjet check book: confirm");
	...
	private AirItinerary vietjetSearchBookIdEditResPost(AirItinerary itinerary,String chkPax1,String hdnInputPax1,int passSize) throws VietjetPayException, Exception{
		String urlEditRes = "https://ameliaweb5.intelisys.ca/VietJet/EditRes.aspx?lang=vi&st=sl&sesid=";
		Connection.Response resPostEditRes = vjPay.postProcess(urlEditRes,vjPay.createPostSearchInfo(chkPax1,hdnInputPax1,viewstate,passSize),sessId);
		URL urlResult = resPostEditRes.url();
		if (!urlResult.toString().contains("EWRevisedItinerary.aspx")){
			logger.error("VietjetSearchBooid: Post_SearchBookIdEditRes Exception");
			throw new Exception("Post_SearchBookIdEditRes Exception");
		}		
		
		Document doc = Jsoup.parse(resPostEditRes.body(), "UTF-8");	
		// <td>0987139444 (mobile)</td>
		String mobible = doc.select("#EditItinerary > div:nth-child(11) > div:nth-child(2) > div:nth-child(2) > table:nth-child(6) > tbody > tr:nth-child(5) > td").text();
		System.out.println(mobible); 
		return itinerary;
	}
	...
	public BookResult vietjetSearchBookId(AirItinerary itinerary) throws Exception {
		...
		int passSize = itinerarySearch.getPassengerInfos().size();
		
		vietjetSearchBookIdEditResPost(itinerarySearch, chkPax1, hdnInputPax1, passSize);
	...
	private AirItinerary extractAirItinerary(String htmlContent) throws Exception {
		...
		try {
			...
			Element chkPaxs = paxEvenTable.select("td").get(0); 
			chkPax1 = chkPaxs.select("input[value]").get(0).attr("value");
			hdnInputPax1 = chkPaxs.select("input[value]").get(1).attr("value");			
...
//			}
	...
	private void loginBuyVietJet() throws Exception{
		...
		viewstate = list.get(0);
		sessId = list.get(1);			
		Connection.Response resPostLogin = vjPay.postProcess(urlLogin,vjPay.createParamsLogin(viewstate, username, password),sessId);
		urlResult = resPostLogin.url();		
		if (!urlResult.toString().contains("AgentOptions.aspx")){
