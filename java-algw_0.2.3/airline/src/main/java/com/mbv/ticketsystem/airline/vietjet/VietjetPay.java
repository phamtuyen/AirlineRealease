...
public class VietjetPay {
	...
	public HashMap<String, String> createParamsLogin(final String viewstate,String user,String pass) throws Exception {	
	...
	// createPostSearchInfo
	
	public HashMap<String, String> createPostSearchInfo(String chkPax1,String hdnInputPax1,final String viewstate,int passSize) throws  VietjetPayException {
		try {
			HashMap<String, String> params = new HashMap<String, String>() {
				{
					put("__VIEWSTATE", viewstate);
					put("SesID", "");
					put("DebugID", "25");		
					put("button", "newitinerary");				
				}
			};					
			params.put("chkPax1", chkPax1);
			params.put("hdnInputPax1", hdnInputPax1);
			
			for(int i = 2;i <= passSize;i++){
				long inputPax = Long.parseLong(hdnInputPax1);
				params.put("hdnInputPax1", ""+inputPax+1);
			}		
			params.put("gridEditSegments", "1");			
			return params;
		} catch (Exception ex) {
